...
class Dense:
    ...
    def forward(self, input):
        self.input = input
        # Add Code Here
        fwd = self.weights.dot(self.input) + self.bias
        return fwd
    ...
    def backward(self, dE_dY, learning_rate):
        # Add Code Here
        dE_dW = dE_dY.dot(self.input.T)
        dE_dX = dE_dY.T.dot(self.weights)
        dE_dB = dE_dY
...
class Activation:
    ...
    def backward(self, dE_dY, learning_rate):
        # Add Code Here
        act_grad = self.activation_grad(self.activation(self.input))
        dE_dX = dE_dY.T * act_grad
        return dE_dX
...
class Sigmoid(Activation):
    def __init__(self):
        ...
        def sigmoid(x):
            ...
            act = []
            for element in x:
                phi = 1 / (1 + math.exp(-element))
                act.append(phi)
            act = np.array(act)
            return np.reshape(act, [len(act), 1])
...
def loss_cross_entropy(y_true, y_pred):
    tmp = -np.multiply(y_true, np.log(y_pred)) - np.multiply(1 - y_true, np.log(1 - y_pred))
    loss = np.sum(tmp)
    return loss
...
def mse(y_true, y_pred):
    # Add Code Here
    m = 1 / len(y_true)
    loss = 1 / len(y_true) * np.sum(np.power(y_pred - y_true, 2))
    return loss


def mse_grad(y_true, y_pred):
    # Add Code Here
    lossGrad = 2 / len(y_true) * (y_true - y_pred)
    return lossGrad
